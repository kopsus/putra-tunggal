...
const ChatBox: FC<{ room: RoomType, role: "Dokter" | "User", h?: string }> = ({ room, role, h }) => {
    ...
    return (
        <div
            className={`flex relative w-full ${h ?? 'h-screen'} lg:pl-5 flex-col gap-5`}
        >
            ...
            <div className="flex flex-col-reverse gap-2 overflow-hidden overflow-y-scroll h-full pb-[4em]">
                {
                    chats.map((chat, index) => (
                        <div key={index} className={`${user?.user.id === chat.senderId ? "flex justify-end" : "flex justify-start"}`}>
            ...
            <div className="fixed right-0 lg:absolute w-full bottom-0 border bg-white flex items-center justify-between p-[1.2em]">
                ...
                <div className="flex items-center gap-x-2 text-black h-5">
                    <div className="w-5 h-5 rounded-full flex justify-center items-center">
                        ...
                        <IoMdSend className="h-full w-full cursor-pointer" onClick={(handleSendMessage)} />
                        {/* ) : ( */}
                        <Dialog>
                            <DialogTrigger>
                                <div ref={triggerRef}>
